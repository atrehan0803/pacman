...
def constructBayesNet(gameState: hunters.GameState):
    ...
    pacDomain = [(x,y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    ghost0Domain = [(x,y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    ghost1Domain = [(x,y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    obs0Domain = range(X_RANGE + Y_RANGE + MAX_NOISE - 1)
    obs1Domain = range(X_RANGE + Y_RANGE + MAX_NOISE - 1)

    variableDomainsDict[PAC] = pacDomain
    variableDomainsDict[GHOST0] = ghost0Domain
    variableDomainsDict[GHOST1] = ghost1Domain
    variableDomainsDict[OBS0] = obs0Domain
    variableDomainsDict[OBS1] = obs1Domain
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        estDistance = manhattanDistance(pacmanPosition, ghostPosition)
        return busters.getObservationProbability(noisyDistance, estDistance)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            noisy_given_real = self.getObservationProb(observation, pacmanPosition, pos, jailPosition)
            beliefs[pos] *= noisy_given_real
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            new_beliefs[pos] = sum(
                transition[prev_position][pos] * prev_beliefs[prev_position]
                for prev_position in prev_beliefs
            )
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        particles_per_position = len(self.legalPositions) / self.numParticles

        count = 0
        particles_per_position_actual = 0
        for p in self.legalPositions:
            while particles_per_position_actual < count + 1:
                self.particles.append(p)
                particles_per_position_actual += particles_per_position
            count += 1
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        w = [self.getObservationProb(observation, gameState.getPacmanPosition(), particle, self.getJailPosition()) for particle in self.particles]
        if sum(w) == 0:
            self.initializeUniformly(gameState)
            return
        normalized = [weight / sum(w) for weight in w]
        particlesArray = []
        for _ in range(self.numParticles):
            index = random.choices(range(len(self.particles)), weights=normalized)[0]
            particlesArray.append(self.particles[index])
        self.particles = particlesArray
        "*** END YOUR CODE HERE ***"
